...
Where `{brand_name_titlecase}` is the brand name in title case (e.g. "Apple", "Tesla Motors") used as the folder name, and `{slug}` is the brand name lowercased with spaces replaced by underscores and special characters removed (used for filenames).
...
5. Save JSON to `clients/{brand_name_titlecase}/json/`
6. Run the build_excel.py script to generate the branded Excel workbook
7. Save Excel to `clients/{brand_name_titlecase}/`
...
5. Create a summary JSON at `clients/{brand_name_titlecase}/{slug}_analysis_summary.json` containing:
